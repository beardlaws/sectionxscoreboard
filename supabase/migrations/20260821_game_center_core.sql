-- Game Center v1 foundation: additive schema for scoring detail, sport-aware stats, and photo/athlete relationships.
...
create index if not exists game_period_scores_game_id_idx on public.game_period_scores(game_id);
...
create index if not exists stat_definitions_sport_id_idx on public.stat_definitions(sport_id, sort_order);
...
create index if not exists game_athlete_stats_game_id_idx on public.game_athlete_stats(game_id);
create index if not exists game_athlete_stats_athlete_id_idx on public.game_athlete_stats(athlete_id);
...
create index if not exists photo_athletes_athlete_id_idx on public.photo_athletes(athlete_id);
...
create policy "Public read game period scores" on public.game_period_scores for select using (true);
drop policy if exists "Public read stat definitions" on public.stat_definitions;
create policy "Public read stat definitions" on public.stat_definitions for select using (active = true);
drop policy if exists "Public read game team stats" on public.game_team_stats;
create policy "Public read game team stats" on public.game_team_stats for select using (true);
drop policy if exists "Public read game athlete stats" on public.game_athlete_stats;
create policy "Public read game athlete stats" on public.game_athlete_stats for select using (true);
drop policy if exists "Public read photo athlete tags" on public.photo_athletes;
create policy "Public read photo athlete tags" on public.photo_athletes for select using (
  exists (select 1 from public.photos p where p.id = photo_id and p.approved = true)
);
