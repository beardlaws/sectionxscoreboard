...
const MAX_FILE_SQL_BYTES = 220_000
const MAX_STATEMENT_BYTES = 55_000
const JSON_FIELD_BUDGET = 12_000
...
const compactableJsonFields = {
  arbiter_game_links: ['source_payload'],
  arbiter_sync_actions: ['details'],
  arbiter_health_checks: ['changes', 'quarantines'],
  arbiter_roster_freshness: ['evidence'],
  admin_exception_resolutions: ['evidence'],
}

function sqlValue(v, isBoolean = false) {
...
function upsert(t, row) {
  const vals = t.columns.map(c => sqlValue(row[c], t.booleans.includes(c)))
  const updates = t.columns.filter(c => !t.key.includes(c)).map(c => `"${c}"=excluded."${c}"`).join(',')
  return `INSERT INTO "${t.name}" (${t.columns.map(c => `"${c}"`).join(',')}) VALUES (${vals.join(',')}) ON CONFLICT (${t.key.map(c => `"${c}"`).join(',')}) DO UPDATE SET ${updates};`
}

function execSql(file) {
  execFileSync('npx', ['wrangler','d1','execute',DB_NAME,'--remote','--config',WRANGLER_CONFIG,'--file',file,'--yes'], {stdio:'inherit', env:process.env})
}

function compactArray(value, field, budget = JSON_FIELD_BUDGET) {
  if (!Array.isArray(value)) return value
  const encoded = JSON.stringify(value)
  if (Buffer.byteLength(encoded) <= budget) return value

  const kept = []
  let bytes = 2
  const markerReserve = 420
  for (const item of value) {
    const itemBytes = Buffer.byteLength(JSON.stringify(item)) + 1
    if (bytes + itemBytes > budget - markerReserve) break
    kept.push(item)
    bytes += itemBytes
  }
  const retainedItems = kept.length
  kept.push({
    migration_compacted: true,
    field,
    original_items: value.length,
    retained_items: retainedItems,
    note: 'Historical payload compacted for Cloudflare D1 migration',
  })
  return kept
}

function compactObject(value, field, budget = JSON_FIELD_BUDGET) {
...
function normalizeRowForD1(t, sourceRow) {
  let row = {...sourceRow}
  let statement = upsert(t, row)
  if (Buffer.byteLength(statement + '\n') <= MAX_STATEMENT_BYTES) return row

  const fields = compactableJsonFields[t.name] || []
  for (const field of fields) {
    const value = row[field]
    row[field] = Array.isArray(value)
      ? compactArray(value, field)
      : compactObject(value, field)
  }

  statement = upsert(t, row)
  let statementBytes = Buffer.byteLength(statement + '\n')
  if (statementBytes <= MAX_STATEMENT_BYTES) {
    console.log(`[D1 Arbiter seed] compacted oversized ${t.name} row ${sourceRow.id || sourceRow.arbiter_game_id || sourceRow.team_id || 'unknown'} from ${Buffer.byteLength(upsert(t, sourceRow) + '\n')} to ${statementBytes} bytes`)
    return row
  }

  // Health checks are historical diagnostics. If both arrays still make a statement too large,
  // preserve the identifying/status fields and replace the detail arrays with explicit markers.
  if (t.name === 'arbiter_health_checks') {
    for (const field of ['changes','quarantines']) {
      const original = sourceRow[field]
      row[field] = [{
        migration_compacted: true,
        field,
        original_items: Array.isArray(original) ? original.length : null,
        note: 'Historical diagnostic detail omitted from D1 seed because the source row exceeded D1 SQL statement limits',
      }]
    }
    statement = upsert(t, row)
    statementBytes = Buffer.byteLength(statement + '\n')
    if (statementBytes <= MAX_STATEMENT_BYTES) return row
  }

  throw new Error(`${t.name} row ${sourceRow.id || sourceRow.arbiter_game_id || sourceRow.team_id || 'unknown'} still exceeds safe D1 statement size (${statementBytes} bytes)`)
}

function sqlChunks(t, rows) {
  const chunks = []
  let current = []
  let bytes = Buffer.byteLength('PRAGMA foreign_keys = ON;\n')

  for (const sourceRow of rows) {
    const row = normalizeRowForD1(t, sourceRow)
    const statement = upsert(t, row)
    const statementBytes = Buffer.byteLength(statement + '\n')
    if (statementBytes > MAX_STATEMENT_BYTES) throw new Error(`${t.name} contains a statement too large for safe D1 seeding (${statementBytes} bytes)`)
    if (current.length && (current.length >= MAX_ROWS_PER_FILE || bytes + statementBytes > MAX_FILE_SQL_BYTES)) {
      chunks.push(current)
      current = []
      bytes = Buffer.byteLength('PRAGMA foreign_keys = ON;\n')
    }
...
try {
  ...
  if (schools.rows.length) {
    const file = '/tmp/sectionx-arbiter-schools.sql'
    writeFileSync(file, ['PRAGMA foreign_keys = ON;', ...schools.rows.map(r => `UPDATE schools SET arbiter_entity_id=${sqlValue(r.arbiter_entity_id)},arbiter_school_url=${sqlValue(r.arbiter_school_url)} WHERE id=${sqlValue(r.id)};`)].join('\n'))
  ...
  for (const t of tables) {
    ...
    console.log(`[D1 Arbiter seed] ${t.name}: ${result.rows.length}`)
    const chunks = sqlChunks(t, result.rows)
    for (let i = 0; i < chunks.length; i++) {
      const file = `/tmp/sectionx-${t.name}-${i}.sql`
      writeFileSync(file, ['PRAGMA foreign_keys = ON;', ...chunks[i]].join('\n'))
      try { execSql(file) } finally { try { unlinkSync(file) } catch {} }
    }
  }
  ...
  console.error('[D1 Arbiter seed] FAILED:', error instanceof Error ? error.message : error)
  process.exitCode = 1
} finally {
