...
function schoolKey(v:unknown){
  let c=clean(v)
  c=c.replace(/\bogdensburg free academy\b/g,'ogdensburg').replace(/\bofa\b/g,'ogdensburg')
  c=c.replace(/\bst[ .]?lawrence\b/g,'st lawrence')
  for(const s of suffixes)c=c.replace(new RegExp(`\\b${s.replace(/ /g,'\\s+')}\\b`,'g'),' ')
  return c.replace(/\b(boys|girls|varsity|soccer|football|volleyball|basketball|hockey|softball|baseball|lacrosse|swimming|cross country)\b/g,' ').replace(/\s+/g,' ').trim()
}
function sportKey(v:unknown){return clean(v).replace(/\b(boys|girls|mens|womens|varsity)\b/g,'').replace(/\s+/g,' ').trim()}

export function parseScoreText(text:string):{records:ScoreRecord[],errors:string[]}{
  const lines=String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean)
  const records:ScoreRecord[]=[];const errors:string[]=[]
  if(!lines.length)return{records,errors:['No score rows were provided.']}
  const delim=(line:string)=>line.includes('\t')?'\t':line.includes('|')?'|':','
  const firstParts=lines[0].split(delim(lines[0])).map(x=>clean(x))
  ...
  for(let i=start;i<lines.length;i++){
    const d=delim(lines[i]);const parts=lines[i].split(d).map(x=>x.trim())
    let date='',sport='',away='',home='',awayRaw='',homeRaw=''
    if(hasHeader){
      date=pick(parts,['date'],0);sport=pick(parts,['sport'],1);away=pick(parts,['away','away team'],2);awayRaw=pick(parts,['away score'],3);home=pick(parts,['home','home team'],4);homeRaw=pick(parts,['home score'],5)
    }else{
      if(parts.length<6){errors.push(`Line ${i+1}: expected Date, Sport, Away, Away Score, Home, Home Score.`);continue}
      ;[date,sport,away,awayRaw,home,homeRaw]=parts
    }
    const normalizedDate=dateOnly(date),as=scoreNum(awayRaw),hs=scoreNum(homeRaw)
    if(!normalizedDate||!away||!home||as===null||hs===null){errors.push(`Line ${i+1}: could not parse a complete final score.`);continue}
...
export async function previewScores(records:ScoreRecord[],source:ScoreSource){
  ...
  if(!dates.length)return{source,summary:{received:0,matched:0,safe:0,verified:0,conflicts:0,ambiguous:0,unmatched:0},rows:[]}
  ...
  const [{data:games,error:ge},{data:teams,error:te},{data:schools,error:se},{data:sports,error:spe},{data:ext,error:ee}]=await Promise.all([
    db.from('games').select('id,game_date,game_time,sport_id,home_team_id,away_team_id,external_home_opponent_id,external_away_opponent_id,home_score,away_score,status,source,verification_status').gte('game_date',start).lte('game_date',end),
  ...
  const rows=records.map((r,index)=>{
    const aKey=schoolKey(r.away),hKey=schoolKey(r.home),sKey=sportKey(r.sport)
    const sameDate=(games||[]).filter((g:any)=>g.game_date===r.date)
    const candidates=sameDate.filter((g:any)=>{
      if(sKey){const s=sportById.get(g.sport_id) as any;const dbSport=sportKey(s?.sport_name||s?.slug);if(dbSport&&dbSport!==sKey)return false}
    ...
    const g=candidates[0],dbHomeName=sideName(g,true),dbAwayName=sideName(g,false),sameOrientation=schoolKey(dbHomeName)===hKey&&schoolKey(dbAwayName)===aKey
    ...
    else if(clean(g.source)===clean(source)||clean(g.source).startsWith('arbiter')){bucket='safe-update';safeToApply=true}
    const sport=sportById.get(g.sport_id) as any
    return{index,...r,gameId:g.id,bucket,safeToApply,matched:{home:dbHomeName,away:dbAwayName,sport:sport?.sport_name||null,currentHome:dbHome,currentAway:dbAway,currentStatus:g.status,currentSource:g.source},desired:{home:desiredHome,away:desiredAway,status:'Final'}}
  })
...
export async function applyPreviewRows(rows:any[],source:ScoreSource){
  const db=createAdminClient();let updated=0,skipped=0,failed=0
  const actions:any[]=[]
  for(const row of rows){
    if(!row?.gameId||!row?.safeToApply||!['safe-fill','safe-update'].includes(row.bucket)){skipped++;continue}
    try{
      const {data:game,error:readError}=await db.from('games').select('id,home_score,away_score,source').eq('id',row.gameId).single();if(readError||!game)throw new Error(readError?.message||'Game not found')
      const currentBlank=game.home_score==null&&game.away_score==null
      const sourceOwned=clean(game.source)===clean(source)||clean(game.source).startsWith('arbiter')
      if(!currentBlank&&!sourceOwned){skipped++;actions.push({gameId:row.gameId,outcome:'conflict-protected'});continue}
      const {error}=await db.from('games').update({home_score:Number(row.desired.home),away_score:Number(row.desired.away),status:'Final',verification_status:'Reported',source,updated_at:new Date().toISOString()}).eq('id',row.gameId)
      if(error)throw new Error(error.message)
      updated++;actions.push({gameId:row.gameId,outcome:'updated'})
