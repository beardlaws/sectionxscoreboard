...
export async function recordScheduleHealthCheck(seasonId:string){
  ...
  const {data:check,error}=await db.from('arbiter_health_checks').insert({season_id:audit.season.id,status,summary,changes,quarantines}).select('id,created_at').single()
  if(error)throw new Error(`Could not record health check: ${error.message}`)
  return{checkId:check.id,createdAt:check.created_at,status,summary,changes,quarantines,audit}
}
