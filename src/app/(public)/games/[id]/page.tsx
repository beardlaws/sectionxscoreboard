...
export default async function GamePage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    'location': {
      '@type': 'Place',
      'name': game.location || 'Section X, Northern New York',
    },
    'homeTeam': { '@type': 'SportsTeam', 'name': homeName },
    'awayTeam': { '@type': 'SportsTeam', 'name': awayName },
    'sport': game.sport?.sport_name || 'High School Sports',
    'organizer': {
      '@type': 'Organization',
      'name': 'Section X Athletic Association',
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              {(isFinal || game.status === 'Live') && (
                <span className={`score-digit text-4xl font-bold ${awayWins ? 'text-white' : 'opacity-50'}`}>
                  {game.away_score ?? '-'}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {(isFinal || game.status === 'Live') && (
                <span className={`score-digit text-4xl font-bold ${homeWins ? 'text-white' : 'opacity-50'}`}>
                  {game.home_score ?? '-'}
                </span>
              )}
            </div>
