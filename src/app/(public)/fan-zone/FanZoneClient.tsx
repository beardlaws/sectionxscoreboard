...
export default function FanZoneClient({sports,teams,schools}:{sports:Sport[];teams:Team[];schools:{id:string;school_name:string}[]}){
  ...
  return <div>
    ...
    {tab==='plays'?<form onSubmit={submitPlay} className="rounded-3xl p-5 sm:p-7 space-y-4" style={{background:'rgba(255,255,255,.025)',border:'1px solid rgba(255,255,255,.07)'}}>
      ...
      <div className="text-[10px] uppercase tracking-[.2em] text-yellow-300/70 font-black">The fans have spoken</div><h2 className="text-2xl font-black text-white mt-1">Build Your Top 5</h2><p className="text-sm text-white/40 mt-2">Rank the teams you know. Submit up to five, or fewer when a class has fewer teams. Your ballot can be updated anytime this week.</p>
      <div className="grid sm:grid-cols-2 gap-3 mt-5"><label className="text-xs text-white/45">Sport<select value={sportId} onChange={e=>chooseSport(e.target.value)} className="input w-full mt-1">{sports.map(s=><option key={s.id} value={s.id}>{s.sport_name}</option>)}</select></label><label className="text-xs text-white/45">Group<select value={currentGroup?.value||'All'} onChange={e=>{setGroup(e.target.value);setRanked([])}} className="input w-full mt-1">{groups.map(g=><option key={g.value} value={g.value}>{g.label}</option>)}</select></label></div>
