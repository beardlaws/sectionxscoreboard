...
export default function ContributorClient() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!user) { setSignedIn(false); setProfile(null); return }
      setSignedIn(true)

      const { data: p, error: profileError } = await supabase.from('contributor_profiles').select('*').eq('user_id', user.id).maybeSingle()
