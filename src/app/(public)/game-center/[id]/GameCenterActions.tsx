    ...
    <div className="flex flex-wrap items-center justify-center gap-3">
      {context?.game?.live && (
        <div className="inline-flex items-center gap-2 rounded-xl border border-yellow-300/20 bg-yellow-300/[0.06] px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-yellow-200">
          <Radio size={13} className="animate-pulse" /> Live updates on · 20s refresh
        </div>
      )}

      {context?.awayTeam && <FollowButton targetType="team" targetId={context.awayTeam.id} targetName={context.awayTeam.name} compact />}
      {context?.homeTeam && <FollowButton targetType="team" targetId={context.homeTeam.id} targetName={context.homeTeam.name} compact />}
