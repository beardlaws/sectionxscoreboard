...
export async function POST(req:NextRequest){
  ...
  if(!body?.sportId||!body?.groupType||!body?.groupValue||unique.length!==5)return NextResponse.json({ok:false,error:'Choose exactly five different teams.'},{status:400})
  const db=createAdminClient()
  const {data:teams}=await db.from('teams').select('id,sport_id').in('id',unique)
  if((teams||[]).length!==5||(teams||[]).some((t:any)=>t.sport_id!==body.sportId))return NextResponse.json({ok:false,error:'Invalid teams for this sport.'},{status:400})
  ...
  const points=new Map<string,number>(),votes=new Map<string,number>()
  ...
  const results=ids.map(id=>{const t:any=byId.get(id),school=Array.isArray(t?.school)?t.school[0]:t?.school;return{teamId:id,teamName:school?.school_name||t?.team_name||'Team',slug:school?.slug||null,points:points.get(id)||0,votes:votes.get(id)||0}}).sort((a,b)=>b.points-a.points||b.votes-a.votes||a.teamName.localeCompare(b.teamName))
