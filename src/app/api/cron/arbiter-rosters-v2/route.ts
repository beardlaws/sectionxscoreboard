...
function findNamedArrayContexts(root: any, names: string[]): ArrayContext[] {
  ...
  while (queue.length) {
    ...
    for (const [key, value] of Object.entries(cur)) {
      if (wanted.has(norm(key)) && Array.isArray(value)) found.push({ values: value as any[], path: [...item.path, key], ancestors: [cur, ...item.ancestors] })
...
function player(p: any) {
  ...
  return {
    ...
    firstName: clean(p?.firstName), lastName: clean(p?.lastName),
    classYear: clean(p?.graduatingClass || p?.classYear || p?.class || p?.grade || p?.graduatingYear || p?.graduationYear),
...
function markerClass(marker: SeasonEvidence, season: any): MarkerClass {
  ...
  const priorStart = currentStart - 1, priorEnd = currentEnd - 1
  const text = marker.value.replace(/[–—]/g, '-')
  const key = norm(marker.key).replace(/ /g, '')
  const pair = text.match(/\b(20\d{2})\s*[-/]\s*(\d{2}|20\d{2})\b/)
  if (pair) {
    const start = Number(pair[1]), end = pair[2].length === 2 ? Number(String(start).slice(0, 2) + pair[2]) : Number(pair[2])
  ...
  const year = years[0], isSyLabel = /\bsy\s*20\d{2}\b/i.test(text), isAcademicYearKey = ['schoolyear', 'academicyear', 'seasonyear'].includes(key)
  if (isSyLabel) {
...
function classifyContext(context: ArrayContext, season: any): { classification: ContextClass; evidence: SeasonEvidence[]; nearestScope: number | null } {
  ...
  const hasCurrent = nearest.some(x => x.cls === 'current'), hasPrior = nearest.some(x => x.cls === 'prior')
...
function freshness(roster: any[], previousNames: string[], season: any, rosterContext: ArrayContext | null) {
  ...
  if (contextMeta.classification === 'current') return { status: 'current-verified', verified: true, reason: `Selected roster instance has nearest Arbiter metadata matching the active academic year at scope ${contextMeta.nearestScope}.`, evidence, priorOverlap, classification: 'current-academic-year-instance' }
  if (contextMeta.classification === 'prior') return { status: 'review-needed', verified: false, reason: `Selected roster instance points to the prior academic year at scope ${contextMeta.nearestScope}.`, evidence, priorOverlap, classification: 'prior-season-roster' }
  if (contextMeta.classification === 'ambiguous') return { status: 'review-needed', verified: false, reason: `Selected roster instance has conflicting current/prior academic-year metadata at scope ${contextMeta.nearestScope}.`, evidence, priorOverlap, classification: 'mixed-nearest-season-evidence' }
  return { status: 'current-verified', verified: true, reason: 'Non-empty roster returned from the stable current-season Arbiter team identity with no prior-year marker or exact prior-season carryover.', evidence, priorOverlap, classification: 'current-linked-team-roster' }
}

async function updateRun(db: any, runId: string | null, patch: any, status?: string, finished = false) {
  if (!runId) return
  await db.prepare(`UPDATE arbiter_roster_automation_runs SET summary=?${status ? ',status=?' : ''}${finished ? ',finished_at=?' : ''} WHERE id=?`)
    .bind(JSON.stringify(patch), ...(status ? [status] : []), ...(finished ? [nowIso()] : []), runId).run()
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await db.prepare('INSERT INTO arbiter_roster_automation_runs (id,trigger_source,status,summary,started_at) VALUES (?,?,?,?,?)')
      .bind(runId, internalRepair ? 'emergency-repair' : 'cloudflare-roster-cron', 'running', JSON.stringify(runSummary), nowIso()).run()

    const season: any = await db.prepare('SELECT id,name,season_type,year,is_active FROM seasons WHERE is_active=1 ORDER BY year DESC LIMIT 1').first()
    if (!season) throw new Error('No active season found.')
    await db.prepare('UPDATE arbiter_roster_automation_runs SET season_id=? WHERE id=?').bind(season.id, runId).run()

    const previousSeason: any = await db.prepare('SELECT id,name,season_type,year FROM seasons WHERE season_type=? AND year=? LIMIT 1').bind(season.season_type, Number(season.year) - 1).first()
    const [teamSeasonResult, teamResult, linkResult] = await Promise.all([
      db.prepare('SELECT team_id FROM team_seasons WHERE season_id=? AND active_for_season=1').bind(season.id).all(),
      db.prepare('SELECT id,team_name,level,active,school_id FROM teams WHERE active=1').all(),
      db.prepare('SELECT team_id,arbiter_team_id,arbiter_school_id,last_seen_at FROM arbiter_team_links').all(),
    ])
    const teamSeasons = teamSeasonResult.results || [], teams = teamResult.results || [], links = linkResult.results || []
    ...
    const previousRows: any[] = previousSeason && teamIds.length ? (await db.prepare(`SELECT re.team_id,a.display_name FROM roster_entries re JOIN athletes a ON a.id=re.athlete_id WHERE re.season_id=? AND re.active=1 AND re.team_id IN (${placeholders(teamIds.length)})`).bind(previousSeason.id, ...teamIds).all()).results || [] : []
    const previousByTeam = new Map<string, string[]>()
    for (const row of previousRows) {
      const list = previousByTeam.get(row.team_id) || []
      if (row.display_name) list.push(row.display_name)
      previousByTeam.set(row.team_id, list)
    }

    runSummary = { ...runSummary, season: season.name, previousSeason: previousSeason?.name || null, policy: POLICY, progress: { ...runSummary.progress, phase: 'scanning', total: varsity.length, heartbeatAt: nowIso() } }
    await updateRun(db, runId, runSummary)

    const audits: any[] = [], verifiedPayloads: any[] = [], failures: any[] = [], quarantines: any[] = []
    for (let i = 0; i < varsity.length; i += BATCH_SIZE) {
      ...
      for (const item of results) {
        ...
        audits.push({ team_id: item.team.id, season_id: season.id, arbiter_team_id: Number(item.link.arbiter_team_id), status: check.verified ? 'current-verified' : check.status, verified: check.verified, reason: check.reason, incoming_count: roster.length, previous_count: (previousByTeam.get(item.team.id) || []).length, previous_overlap: Number(check.priorOverlap.toFixed(4)), evidence: { policy: POLICY, rosterPath: rosterContext?.path || null, seasonFields: check.evidence, classification: check.classification || check.status, academicYear: academicYear(season) }, checked_at: nowIso() })
        if (check.verified) verifiedPayloads.push({ team_id: item.team.id, school_id: item.team.school_id, season_id: season.id, source_url: null, roster_found: rosterContext !== null, coaches_found: coachesContext !== null, roster, coaches })
    ...
    for (const audit of audits) {
      await db.prepare(`INSERT INTO arbiter_roster_freshness (team_id,season_id,arbiter_team_id,status,verified,reason,incoming_count,previous_count,previous_overlap,evidence,checked_at) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(team_id,season_id) DO UPDATE SET arbiter_team_id=excluded.arbiter_team_id,status=excluded.status,verified=excluded.verified,reason=excluded.reason,incoming_count=excluded.incoming_count,previous_count=excluded.previous_count,previous_overlap=excluded.previous_overlap,evidence=excluded.evidence,checked_at=excluded.checked_at`)
        .bind(audit.team_id, audit.season_id, audit.arbiter_team_id, audit.status, audit.verified ? 1 : 0, audit.reason, audit.incoming_count, audit.previous_count, audit.previous_overlap, JSON.stringify(audit.evidence), audit.checked_at).run()
    }

    let imported = 0, actions: any[] = []
    if (verifiedPayloads.length) {
      runSummary = { ...runSummary, progress: { ...runSummary.progress, phase: 'importing', processed: 0, importTotal: verifiedPayloads.length, heartbeatAt: nowIso() } }
      await updateRun(db, runId, runSummary)
      const bySchool = new Map<string, any[]>()
      for (const payload of verifiedPayloads) { const key = clean(payload.school_id) || payload.team_id, group = bySchool.get(key) || []; group.push(payload); bySchool.set(key, group) }
      const schoolGroups = [...bySchool.values()], allResults: any[] = []
      for (let i = 0; i < schoolGroups.length; i += IMPORT_SCHOOL_CONCURRENCY) {
        ...
        const waveResults = await Promise.all(wave.map(async group => {
          const importReq = new NextRequest('http://sectionx.internal/api/admin/arbiter-rosters', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ teams: group }) })
          const response = await importRosters(importReq), result: any = await response.json()
          if (!response.ok || result?.success === false) throw new Error(result?.error || 'Verified roster import failed.')
          return Array.isArray(result?.results) ? result.results : []
        }))
        allResults.push(...waveResults.flat()); imported = allResults.length
        runSummary = { ...runSummary, teamsUpdated: imported, progress: { ...runSummary.progress, phase: 'importing', processed: imported, importTotal: verifiedPayloads.length, heartbeatAt: nowIso() } }
        await updateRun(db, runId, runSummary)
      }
      actions = allResults.map((x: any) => ({ teamId: x.team_id || x.teamId, rosterImported: x.rosterImported ?? null, rosterDeactivated: x.rosterDeactivated ?? null, coachesImported: x.coachesImported ?? null, coachesDeactivated: x.coachesDeactivated ?? null, errors: x.errors || [] }))
    }
    ...
    return NextResponse.json({ ok: true, runId, season: season.name, previousSeason: previousSeason?.name || null, policy: POLICY, scanned: varsity.length, verified: verifiedPayloads.length, held: quarantines.length, imported, failures, statuses })
  } catch (error) {
    ...
    await updateRun(db, runId, runSummary, 'failed', true).catch(() => null)
