...
export async function GET(req:NextRequest){
  ...
  try{
    ...
    for(const season of cycle){
      try{
        const check=await recordScheduleHealthCheck(season.id)
        results.push({season:season.name,seasonId:season.id,status:check.status,summary:check.summary,checkId:check.checkId})
      }catch(error){
        results.push({season:season.name,seasonId:season.id,status:'error',error:error instanceof Error?error.message:String(error)})
      }
    }
    const ok=results.every((r:any)=>r.status!=='error'&&r.status!=='blocked')
    return NextResponse.json({ok,readOnly:true,checkedAt:new Date().toISOString(),results},{status:ok?200:207})
  }catch(error){
