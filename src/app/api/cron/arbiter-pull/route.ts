  ...
  try{
    ...
    const updateExisting=async(row:any,gameId:string)=>{
      ...
      if(!current.scheduleOverride){
        ...
        if((row.driftReasons||[]).some((r:string)=>r==='home-away-reversed'||r==='opponent-orientation-mismatch')&&current.homeScore==null&&current.awayScore==null&&clean(current.status)!=='final'){
