...
export async function GET(req:NextRequest){
  ...
  try{
    ...
    const linkRow=async(row:any,gameId:string)=>{
      const {error}=await db.from('arbiter_game_links').upsert({
    ...
    const updateExisting=async(row:any,gameId:string)=>{
      ...
      if(Object.keys(patch).length){
        ...
        if(error)throw new Error(`Schedule update failed for ${gameId}: ${error.message}`)
        scheduleUpdated++
        scheduleActions.push({arbiterGameId:row.uniqueGameId,gameId,action:'updated',patch,driftReasons:row.driftReasons||[]})
      }
    ...
    const createSafeGame=async(row:any)=>{
      ...
      const homeInternal=row.home?.kind==='internal'?row.home.id:null
      const awayInternal=row.away?.kind==='internal'?row.away.id:null
      const homeExternal=row.home?.kind==='external'?await ensureExternal(row.home):null
      const awayExternal=row.away?.kind==='external'?await ensureExternal(row.away):null
      ...
      const h=homeInternal?`t:${homeInternal}`:`e:${homeExternal}`
      const a=awayInternal?`t:${awayInternal}`:`e:${awayExternal}`
      ...
      if(row.time){
        const exact=teamMatches.filter((g:any)=>String(g.game_time||'').slice(0,5)===row.time)
        if(exact.length>1)throw new Error('Multiple same-team same-time games exist; automatic create quarantined.')
        duplicate=exact[0]||null
      }else if(teamMatches.length){
        throw new Error('Same teams already play on this date and Arbiter has no reliable time; automatic create quarantined.')
      }
      if(duplicate){
        row.existing={id:duplicate.id,gameTime:duplicate.game_time}
        await updateExisting(row,duplicate.id)
        scheduleActions.push({arbiterGameId:row.uniqueGameId,gameId:duplicate.id,action:'duplicate-prevented'})
        return
      }
      ...
      scheduleCreated++
      scheduleActions.push({arbiterGameId:row.uniqueGameId,gameId:created.id,action:'created',bucket:row.bucket})
    }

    for(const row of audit.rows||[]){
      if(row?.uniqueGameId==null||!row.safelyActionable)continue
      try{
        if(['stable-id-match','stable-id-update','exact-match','probable-match'].includes(row.bucket)&&row.existingGameId){
          await updateExisting(row,row.existingGameId)
        }else if(['new-game','external-create'].includes(row.bucket)){
          await createSafeGame(row)
        }
      }catch(error){
        scheduleFailures++
        scheduleActions.push({arbiterGameId:row.uniqueGameId,gameId:row.existingGameId||null,action:'failed',error:error instanceof Error?error.message:String(error)})
      }
    }
    ...
    try{
      ...
      for(const arbiterId of deletedIds(deleted)){
        ...
        if(error){scheduleFailures++;continue}
    ...
    const summary={
      ...
      schedule:{stableLinks:linksRefreshed,updated:scheduleUpdated,created:scheduleCreated,externalCreated,deletedMarked,failed:scheduleFailures,pendingChanges:audit.comparison.pendingChanges||0,quarantined:audit.comparison.quarantined||0,blockers:audit.comparison.trueBlockers||0,actions:scheduleActions.slice(0,75)},
