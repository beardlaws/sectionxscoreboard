...
export default function RosterIntelligence({ rows, source }: { rows: any[]; source?: string }) {
  ...
  const prior = (counts['prior-season-roster'] || 0) + (counts['possible-prior-season-roster'] || 0)
  const awaiting = counts['awaiting-current-roster'] || 0
  const review = (counts['review-needed'] || 0) + (counts['current-probable'] || 0)
  ...
  return <div className="card p-4">
    <div className="flex items-center justify-between gap-2">
      ...
      {source === 'canonical-fallback' && <span className="rounded-full border border-sky-500/20 bg-sky-500/[.06] px-2 py-1 text-[10px] font-bold text-sky-300">CANONICAL DATA</span>}
    </div>
    ...
    {rows.length === 0
      ? <div className="mt-4 rounded border border-red-500/20 bg-red-500/[.05] p-3 text-xs text-red-200">No active-season roster publication population could be reconstructed. Public roster guardrails remain closed; investigate the admin data path before publishing anything.</div>
      : <div className="mt-4 text-[11px]" style={{color:'var(--text-muted)'}}>Publication rule remains fail-closed: active-season Arbiter rows reach fans only after explicit current-season verification.</div>}
  </div>
}
