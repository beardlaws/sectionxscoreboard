...
export default function RosterIntelligence({ rows }: { rows: any[] }) {
  const counts = rows.reduce((acc: Record<string, number>, row: any) => {
    const key = row.freshness_status || 'not-scanned'
    acc[key] = (acc[key] || 0) + 1
    return acc
  }, {})
  const published = rows.filter(r => r.arbiter_roster_public).length
  ...
  const notScanned = counts['not-scanned'] || 0
  const stored = rows.reduce((n, r) => n + Number(r.arbiter_roster_count || 0), 0)
  const scanned = rows.filter(r => r.freshness_status).length
  ...
  return <div className="card p-4">
    ...
    <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>What Arbiter has, what Section X trusts, and what fans are actually allowed to see.</p>

    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 mt-4">
      {[
        [published, 'Current & public'],
    ...
    <div className="mt-4 flex flex-wrap gap-2 text-xs">
      ...
      <span className="inline-flex items-center gap-1 rounded-full border border-sky-500/20 bg-sky-500/[.06] px-2.5 py-1 text-sky-300"><Database size={12} /> {stored} quarantined player rows retained</span>
