...
export default function SpotlightAdmin() {
  ...
  const blank = { title: '', body: '', author: 'Section X Scoreboard', school_slug: '', sport_name: '', published: true, featured: false }
  ...
  async function save() {
    if (!editing?.title || !editing?.body) { setMsg('Title and body are required.'); return }
    setSaving(true)
    if (editing.id) {
      await supabase.from('spotlights').update({
        title: editing.title, body: editing.body, author: editing.author,
        school_slug: editing.school_slug || null, sport_name: editing.sport_name || null,
  ...
  async function toggleFeatured(id: string, current: boolean) {
    ...
    if (!current) await supabase.from('spotlights').update({ featured: false }).neq('id', id)
    ...
    <AdminLayout>
      <div className="p-4 max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-2xl font-black text-white" style={{ fontFamily: 'var(--font-display)' }}>📰 Section X Spotlight</h1>
          {msg && <span className="text-sm text-green-400">{msg}</span>}
        </div>
        <p className="text-slate-400 text-sm mb-5">Write stories, athlete features, and recaps. The featured story appears on the homepage.</p>

        {/* New / Edit form */}
        {editing ? (
          <div className="card p-5 mb-5 space-y-3">
            <p className="text-sm font-bold text-white mb-1">{editing.id ? 'Edit Story' : 'New Story'}</p>
            <div>
              <label className="label">Headline *</label>
              <input className="input w-full" placeholder="Canton's Roiger dominates in Class A championship..."
                value={editing.title} onChange={e => setEditing((p: any) => ({ ...p, title: e.target.value }))} />
            </div>
            <div>
              <label className="label">Story *</label>
              <textarea className="input w-full h-40 resize-none" placeholder="Write the story here..."
                value={editing.body} onChange={e => setEditing((p: any) => ({ ...p, body: e.target.value }))} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Author</label>
                <input className="input w-full" value={editing.author}
              ...
              <div>
                ...
                <input className="input w-full" placeholder="e.g. Baseball"
                  value={editing.sport_name} onChange={e => setEditing((p: any) => ({ ...p, sport_name: e.target.value }))} />
              </div>
            </div>
        ...
        {loading ? <div className="text-center py-8 text-slate-500">Loading...</div> : (
          <div className="space-y-2">
            {stories.length === 0 && <div className="card p-8 text-center text-slate-500">No stories yet.</div>}
            {stories.map(s => (
              <div key={s.id} className="card p-4 flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <p className="font-bold text-white text-sm truncate" style={{ fontFamily: 'var(--font-display)' }}>{s.title}</p>
                  ...
                  <p className="text-xs text-slate-700 mt-1">by {s.author} · {new Date(s.created_at).toLocaleDateString()}</p>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <button onClick={() => setEditing(s)} className="p-1.5 text-slate-400 hover:text-white">
                    ✏️
                  </button>
