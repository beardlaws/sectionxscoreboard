...
export default async function ContributorsAdminPage() {
  ...
  const [{ data: profiles }, { data: games }, { data: pendingUpdates }, { data: assignments }, { data: coverageRequests }] = await Promise.all([
    ...
    db.from('contributor_coverage_requests').select('id,game_id,coverage_role,status,notes,claimed_by,claimed_at,created_at').in('status',['open','claimed']).order('created_at',{ascending:false}),
  ])

  return <div className="space-y-6"><CoverageBoard profiles={profiles||[]} games={games||[]} requests={coverageRequests||[]} /><ContributorAdmin profiles={profiles||[]} games={games||[]} pendingUpdates={pendingUpdates||[]} assignments={assignments||[]} /></div>
}
