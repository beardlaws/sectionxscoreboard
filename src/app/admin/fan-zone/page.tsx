...
export default async function FanZoneAdminPage(){
 ...
 const [{data:plays},{data:snapshots},{data:ballots},{data:staff},{data:teamSeasons}]=await Promise.all([
  ...
  db.from('staff_power_rank_snapshots').select('*').order('week_start',{ascending:false}).limit(100),
  db.from('team_seasons').select('class,division,active_for_season,team:teams(id,sport_id,team_name,school:schools(school_name))').neq('active_for_season',false)
 ])
