...
const EXAMPLE=`Date,Sport,Away,Away Score,Home,Home Score\nYYYY-MM-DD,Girls Soccer,Away School,1,Home School,3`
const readable=(v:string)=>String(v||'').replace(/-/g,' ').replace(/\b\w/g,m=>m.toUpperCase())

export default function ScoreIntelligence(){
  const [source,setSource]=useState('manual-batch'),[text,setText]=useState(''),[preview,setPreview]=useState<any>(null),[loading,setLoading]=useState(false),[applying,setApplying]=useState(false),[error,setError]=useState<string|null>(null),[message,setMessage]=useState<string|null>(null),[exampleLoaded,setExampleLoaded]=useState(false)
  ...
  return <div className="p-4 max-w-6xl space-y-5">
    ...
    <div className="card p-4 space-y-4">
      <div className="flex flex-col md:flex-row gap-3"><label className="flex-1 text-sm"><span style={{color:'var(--text-muted)'}}>Source</span><select value={source} onChange={e=>{setSource(e.target.value);setPreview(null)}} className="w-full mt-1 rounded border border-white/10 bg-black/30 px-3 py-2 text-white">{SOURCES.map(s=><option key={s.id} value={s.id}>{s.label}</option>)}</select></label><button onClick={()=>{setText(EXAMPLE);setPreview(null);setExampleLoaded(true)}} className="admin-action-btn justify-center md:self-end"><ClipboardPaste size={18}/>Load Format Example</button></div>
      {exampleLoaded&&<div className="rounded-lg p-3 border border-amber-500/30 bg-amber-500/10 text-amber-200 text-sm"><AlertTriangle size={16} className="inline mr-2"/><b>FORMAT EXAMPLE ONLY.</b> Replace the placeholder date, schools and score with a verified real result before previewing. Example data is never intended for production entry.</div>}
      <div><div className="text-sm font-semibold text-white mb-1">Paste score rows</div><div className="text-xs mb-2" style={{color:'var(--text-muted)'}}>CSV, tabs, or pipes. Preferred columns: Date, Sport, Away, Away Score, Home, Home Score. Nothing is written during preview.</div><textarea value={text} onChange={e=>{setText(e.target.value);if(e.target.value!==EXAMPLE)setExampleLoaded(false)}} rows={9} className="w-full rounded-lg border border-white/10 bg-black/30 p-3 text-sm text-white font-mono" placeholder={EXAMPLE}/></div>
      <button onClick={runPreview} disabled={loading||!text.trim()||exampleLoaded} className="admin-action-btn justify-center w-full disabled:opacity-40">{loading?<RefreshCw size={18} className="animate-spin"/>:<ShieldCheck size={18}/>} {loading?'Matching existing games…':exampleLoaded?'Replace Example Before Preview':'Preview & Match Results'}</button>
    </div>
    {preview&&<>
