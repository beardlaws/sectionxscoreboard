...
export default function DailyResults() {
  ...
  async function sweepNorthCountrySports() {
    ...
    try {
      ...
      setMessage(
        j.published
          ? `North Country Sports: parsed ${j.parsed}; applied ${j.applied?.updated || 0}; already known/protected ${Math.max((j.parsed || 0) - (j.applied?.updated || 0), 0)}. Score coverage is now ${j.coverage?.percent ?? '—'}%.`
  ...
  return <div className="p-4 max-w-6xl space-y-5">
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
          Game-night and overnight view of what is final, what is still missing, and how close the scoreboard is to complete.
        </p>
      </div>
    ...
    <div className="card p-4 flex flex-col md:flex-row gap-3 md:items-end">
      ...
      <button onClick={() => setDate(easternDate(-1))} className="admin-action-btn justify-center">
        Yesterday
      </button>
    </div>
    ...
    {data && <>
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
        {[
          ...
          ['Score reported', data.summary.reported],
          ['Missing result', data.summary.missing],
          ['Scrimmages', data.summary.scrimmages],
          ['Coverage', `${coverage}%`],
        ].map(([l, v]: any) => <div key={l} className="card p-4">
          <div className={`text-xl font-bold ${l === 'Coverage' && coverage === 100 ? 'text-emerald-300' : 'text-white'}`}>{v}</div>
          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{l}</div>
        </div>)}
      </div>

      <div className={`rounded-xl border p-4 ${coverage === 100 ? 'border-emerald-500/25 bg-emerald-500/10' : 'border-amber-500/25 bg-amber-500/10'}`}>
        <div className={`font-semibold ${coverage === 100 ? 'text-emerald-200' : 'text-amber-200'}`}>
          {coverage === 100
            ? 'Scoreboard complete for all expected official results.'
            : `${data.summary.missing} official result${data.summary.missing === 1 ? '' : 's'} still missing · ${coverage}% complete.`}
        </div>
        <div className="text-xs mt-1 text-white/50">
          Scrimmages and canceled/postponed games do not reduce the completion percentage.
        </div>
      </div>
